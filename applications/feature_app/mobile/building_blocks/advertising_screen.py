...
CLOSE_BUTTON_CENTER_X_IOS = 5
CLOSE_BUTTON_CENTER_Y_IOS = 50
...
class AdvertisingScreen(CommonSideMenuScreen):
    ...
    def navigate(self, home_navigation=False, verify_in_screen=True):
        Logger.get_instance().info(self, 'navigate', START_NAVIGATING_TO_SCREEN %
                                   (self.screen_name, self.id))
        PRINT('     Start navigating to screen "%s" from the side menu' %
              self.screen_name)
        ...
        Logger.get_instance().info(self, 'navigate', FINISHED_NAVIGATING_TO_SCREEN %
                                   (self.screen_name, self.id))
    ...
    def close_interstitial(self):
        ...
        if PLATFORM == PlatformType.IOS:
            ...
            self.__press_element_by_title__(
                CLOSE_INTERSTITIAL_ACCESSIBILITY_ID)
            self.test.driver.tap_by_coordinates(
                CLOSE_BUTTON_CENTER_X_IOS, CLOSE_BUTTON_CENTER_Y_IOS)
        else:
            """
            Workaround:
            ...
            self.test.driver.tap_by_coordinates(
                CLOSE_BUTTON_CENTRE_X, CLOSE_BUTTON_CENTRE_Y)
    ...
    def verify_interstitial_is_displaying(self):
        ...
        if PLATFORM == PlatformType.IOS:
            Logger.get_instance().log_assert(
                ...
                self.test.driver.find_element_by_text(
                    CLOSE_INTERSTITIAL_ACCESSIBILITY_ID, retries=INTERSTITIAL_LOAD_TIMEOUT),
                'Interstitial is not displaying on the screen'
            )
        else:
            Logger.get_instance().log_assert(
                self.test.driver.find_element_by_text(
                    INTERSTITIAL_SCREEN_ID, retries=INTERSTITIAL_LOAD_TIMEOUT),
                'Interstitial is not displaying on the screen'
            )
    ...
    def verify_interstitial_is_not_displaying(self):
        Logger.get_instance().log_assert(
            ...
            not self.test.driver.find_element_by_text(
                CLOSE_INTERSTITIAL_ACCESSIBILITY_ID, retries=3),
    ...
    def __press_element_by_title__(self, title):
        ...
        Logger.get_instance().log_assert(
            element, 'Element with title "%s" not found on the screen' % title)
