...
class WebviewComponent(BaseTest):
    ...
    def test_verify_html_component(self):
        ...
        item_text = 'title_1'
        PRINT('Step 2: Search on the screen for the text "%s" which is found in the html component html page' % item_text)
        element = screen.search_for_item_by_text(item_text)
        Logger.get_instance().log_assert(element, 'Test could not find "%s" in html component' % item_text)
        PRINT('     Step 2.1: item "%s" found on the screen correctly' % item_text)

    def shortDescription(self, test_name) -> str:
        if test_name == 'test_verify_html_component':
            return 'test_verify_html_component:\n' \
